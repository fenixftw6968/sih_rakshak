...
// ---------------------------------------------------------------------------
async function runAgentTask(targetTabId, task, maxSteps = 12, serverUrl = 'http://localhost:8000', localCredentials = {}) {
  ...
  try {
    ...
    while (currentStep < maxSteps && !taskComplete && agentState.isRunning) {
      ...
      if (currentStep === 1) {
        ...
        if (localCredentials && localCredentials.hasCredentials) {
          addAgentLog('✓', 'Credentials protected — real values used locally', 'done');
      ...
      // Check media completion shortcut
      ...
      try {
        ...
        throw new Error(`Cannot connect to backend server at ${serverUrl}. Please ensure the Python server is running.`);
      }
      ...
      // Mask password in technical log
      ...
      if (safeDecisionLog.value && (safeDecisionLog.action === 'FILL_PASSWORD' || /password/i.test(safeDecisionLog.reason))) {
        safeDecisionLog.value = '[REDACTED_PASSWORD]';
      }
      addTechnicalDetail({ step: currentStep, decision: safeDecisionLog });

      // If user stopped in the meantime
      if (!agentState.isRunning) break;

      // Map AI decision to local executable action
  ...
  if (message.type === 'START_AGENT_TASK') {
    (async () => {
      try {
        ...
        const serverUrl = message.serverUrl || 'http://localhost:8000';
        const localCredentials = message.localCredentials || {};

        // Run asynchronously in background service worker
        runAgentTask(tabId, task, maxSteps, serverUrl, localCredentials);
