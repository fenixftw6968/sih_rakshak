...
(function () {
  ...
  const ALLOWED_ACTIONS = new Set(['CLICK', 'TYPE', 'KEY', 'SCROLL', 'WAIT', 'STOP', 'PLAY', 'FILL_CREDENTIALS', 'FILL_EMAIL', 'FILL_PASSWORD']);
  ...
  function findElement(target) {
    ...
    if (typeof target === 'string') {
      const byRakshakId = document.querySelector(`[data-rakshak-id="${target}"]`);
  ...
  function validateAction(actionObj) {
    ...
    // FILL_CREDENTIALS action validation
    if (actionType === 'FILL_CREDENTIALS') {
      const emailTgt = actionObj.emailTarget || (actionObj.target && actionObj.target.emailTarget) || (actionObj.target && actionObj.target.elementId);
      const passTgt = actionObj.passwordTarget || (actionObj.target && actionObj.target.passwordTarget);

      const emailEl = emailTgt ? findElement(emailTgt) : document.querySelector('input[type="email"], input[type="text"], input[name*="user" i], input[name*="email" i], input[id*="user" i], input[id*="email" i]');
      const passEl = passTgt ? findElement(passTgt) : document.querySelector('input[type="password"], input[name*="pass" i], input[id*="pass" i]');

      if (!emailEl && !passEl) {
        return { valid: false, error: 'Neither email/username nor password target input element was found in DOM' };
      }
      return { valid: true, actionType: 'FILL_CREDENTIALS', emailElement: emailEl, passwordElement: passEl };
    }

    if (actionType === 'FILL_EMAIL' || actionType === 'FILL_PASSWORD') {
      let el = null;
      if (actionObj.target && (actionObj.target.elementId || actionObj.target.selector)) {
        el = findElement(actionObj.target);
      } else if (actionObj.elementId) {
        el = findElement(actionObj.elementId);
      } else {
        el = actionType === 'FILL_PASSWORD'
          ? document.querySelector('input[type="password"], input[name*="pass" i]')
      ...
      if (!el || !isElementInteractable(el)) {
        return { valid: false, error: `Target element for ${actionType} not found or not interactable` };
      }
   ...
   */
  function setElementValueSafely(el, textToType, attachedKey = null) {
    if (!el) return false;
    ...
    if (attachedKey) {
      const keyUpper = String(attachedKey).toUpperCase();
    ...
    return Boolean(currentVal && (currentVal === textToType || currentVal.length > 0));
      ...
      if (actionType === 'TYPE') {
        ...
        const verified = setElementValueSafely(el, textToType, actionObj.key);

        return {
          success: true,
          action: 'TYPE',
          message: `Typed text successfully (Verified: ${verified ? 'YES' : 'NO'}${actionObj.key ? `, Key: ${actionObj.key}` : ''})`,
          details: { verified, targetTag: el.tagName }
        };
      }

      // 5b. FILL_CREDENTIALS Action (Local-only credential population)
      if (actionType === 'FILL_CREDENTIALS') {
        const emailEl = validation.emailElement;
        const passEl = validation.passwordElement;
        const emailVal = String(actionObj.emailValue || actionObj.usernameValue || '');
        const passVal = String(actionObj.passwordValue || '');

        let emailOk = false;
        let passOk = false;

        if (emailEl && emailVal) {
          emailOk = setElementValueSafely(emailEl, emailVal);
        }
        if (passEl && passVal) {
          passOk = setElementValueSafely(passEl, passVal);
        }

        const filledParts = [];
        if (emailOk) filledParts.push('email/username');
        if (passOk) filledParts.push('password');

        return {
          success: filledParts.length > 0,
          action: 'FILL_CREDENTIALS',
          message: `Credentials filled locally (${filledParts.join(' & ')} verified)`,
          details: { emailFilled: emailOk, passwordFilled: passOk }
        };
      }

      // 5c. FILL_EMAIL Action
      if (actionType === 'FILL_EMAIL') {
        const el = validation.element;
        const emailVal = String(actionObj.emailValue || actionObj.value || '');
        const ok = setElementValueSafely(el, emailVal, actionObj.key);
        return {
          success: ok,
          action: 'FILL_EMAIL',
          message: `Email/Username filled locally (Verified: ${ok ? 'YES' : 'NO'})`,
          details: { verified: ok }
        };
      }

      // 5d. FILL_PASSWORD Action
      if (actionType === 'FILL_PASSWORD') {
        const el = validation.element;
        const passVal = String(actionObj.passwordValue || actionObj.value || '');
        const ok = setElementValueSafely(el, passVal, actionObj.key);
        return {
          success: ok,
          action: 'FILL_PASSWORD',
          message: `Password filled locally (Verified: ${ok ? 'YES' : 'NO'})`,
          details: { verified: ok }
        };
      }
